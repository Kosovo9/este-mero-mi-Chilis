...
import { CheckIcon, CreditCardIcon, MercadoPagoIcon, PayPalIcon } from './Icons';
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
            ...
            <div className="relative bg-christmas-dark border-2 border-christmas-gold rounded-2xl p-8 max-w-md w-full animate-fadeIn shadow-[0_0_50px_rgba(212,175,55,0.2)]">
                ...
                <div className="flex flex-col items-center text-center">
                    ...
                    <button className="w-full py-3 bg-christmas-gold text-christmas-dark font-bold rounded-full hover:bg-yellow-400 transition-colors">
                        {cta}
                    </button>
                </div>
            </div>
...
export const PricingModal: React.FC<{ isOpen: boolean; onClose: () => void; onPurchase: (credits: number) => void; t: Translation }> = ({ isOpen, onClose, onPurchase, t }) => {
    ...
    const [selectedMethod, setSelectedMethod] = useState<'mp' | 'paypal' | null>(null);
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-8">
                                <button 
                                    ...
                                    className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all gap-2 ${selectedMethod === 'mp' ? 'border-[#009EE3] bg-[#009EE3]/10' : 'border-white/10 hover:border-[#009EE3]/50 bg-white/5'}`}
                                >
                                    <MercadoPagoIcon />
                                    <span className="text-sm font-bold text-white">Mercado Pago</span>
                                </button>
                                <button 
                                    onClick={() => setSelectedMethod('paypal')}
                                    className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all gap-2 ${selectedMethod === 'paypal' ? 'border-[#003087] bg-[#003087]/10' : 'border-white/10 hover:border-[#003087]/50 bg-white/5'}`}
                                >
                                    <PayPalIcon />
                            ...
                            >
                                {selectedMethod ? `${t.modals.payWith} ${selectedMethod === 'mp' ? 'Mercado Pago' : 'PayPal'}` : t.modals.selectMethod}
                            </button>
